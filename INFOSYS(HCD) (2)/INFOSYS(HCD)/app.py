...
DATASET_PATH = r"C:\INFOSYS(HCD) (2)\INFOSYS(HCD)\HCD(Preprocessed) (1).csv"  # Set your dataset path here
...
def load_and_train_model():
    ...
    try:
        ...
        return cat_model, label_encoders
        ...
        unknown_count = data['historical_default'].value_counts().get('Unknown', 0)

        if unknown_count > 0:
        # Determine the number of 'Y's and 'N's to generate
            num_y = unknown_count // 2
            num_n = unknown_count - num_y  # Ensure the total matches the unknown count

    # Create an array of new values
            new_values = ['Y'] * num_y + ['N'] * num_n

    # Shuffle the new values to randomize their order
            np.random.shuffle(new_values)

    # Replace 'Unknown' values with the new values
            unknown_indices = data[data['historical_default'] == 'Unknown'].index
            data.loc[unknown_indices, 'historical_default'] = new_values
        ...
        # Train the model
        cat_model = CatBoostClassifier(iterations=1000, learning_rate=0.1, depth=8, random_state=42, verbose=0)
...
cat_model, label_encoders = load_and_train_model()
